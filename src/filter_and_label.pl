...
# filter.
# Throughout the filter, the variants are labeled with the following categories:
# Confident (Pos, Neg), Unsure (A, B), Remove
#
...
my $dir_temp =$dir_out."/".$name."/"; #Folder for temporary files
...
print "Start variant annotation......\n";
	...
	`zcat $file_gatk | /bin/annovar/convert2annovar.pl -format vcf4 - -includeinfo | grep '^chr' |grep -v '^chrM' > $file_annovar_input`; ##convert variants to annovar format, exclude mutations in chrM or "GL" chrs
	...
	`/bin/annovar/annotate_variation.pl -outfile $file_annovar_gene_header -exonicsplicing -build hg38 $file_annovar_input /bin/annovar/humandb/ -dbtype ensGene`; ##Annotate variant gene location
...
print "Start common SNP filter ......\n";
	...
	`/bin/annovar/annotate_variation.pl -filter -dbtype gnomad30_genome -buildver hg38 -out $file_annovar_gnomad_fil_header $file_annovar_exonic /bin/annovar/humandb/`;
...
#	`/bin/annovar/annotate_variation.pl -filter -dbtype avsnp147 -buildver hg38 -out $file_annovar_dbsnp_fil_header $file_annovar_gnomad_fil /bin/annovar/humandb/`;
	...
	`/bin/annovar/annotate_variation.pl -filter -dbtype generic -genericdbfile hg38_RNAedit.txt -buildver hg38 -out $file_annovar_rnaedit_header $file_annovar_gnomad_fil /bin/annovar/humandb/`;
...
$file_undefined_A = $dir_uns."/Undefined_A.txt";
$file_undefined_B = $dir_uns."/Undefined_B.txt";
#$file_undefined_C = $dir_uns."/Undefined_C.txt";
#$file_undefined_D = $dir_uns."/Undefined_D.txt";
...
open(FUNSA,">$file_undefined_A")||die;
open(FUNSB,">$file_undefined_B")||die;
...
#print $count_dp_qcp{$var_info},"\n";#die;
		if(defined $class_dp_qc{$sample_name}{$var_info}){ ##Variant contained in sample
			...
			if($cp_qcp >= $recur_cut && $cp_qcp < $recur_upper){ ##Variant recurrent
				...
				print FUNSA $sample_name,"\t",$class_dp_qc{$sample_name}{$var_info}; #,"\n";
			}
		}
	}
	foreach $var_info(sort{$count_df_qcf{$b}<=>$count_df_qcf{$a}} keys %count_df_qcf){
		if(defined $class_df_qc{$sample_name}{$var_info}){
			...
			if($cf_qcf==1){
				...
				print FUNSB $sample_name,"\t",$class_df_qc{$sample_name}{$var_info}; #,"\n";
			}
		}
	}
}
